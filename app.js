...
const ataquePorTipo={Mago:'magia',Guerreiro:'espada',Monge:'artes marciais',Ninja:'shuriken'}
...
class Hero{
  ...
  atacar(){return `o ${this.tipo.toLowerCase()} atacou usando ${ataquePorTipo[this.tipo]}`}
  
  poderDeCombate(){
    const h=this.hp/this.hpMax,m=this.mp/this.mpMax,s=this.sp/this.spMax
  ...
  checkLevelUp() {
    ...
    if (newLevel > this.level) {
      ...
      this.combatStats.spellPower += Math.floor(this.combatStats.spellPower * 0.05);
...
function fmt(v,m){return `${v} / ${m}`}
function pct(v,m){return Math.max(0,Math.min(100,(v/m)*100))}

const ui={log(lines=[]){battleLog.innerHTML=lines.map(t=>`<div class="line">${t}</div>`).join('')}}

function updateXP(root,hero){
  const ratio=pct(hero.xp,hero.xpToNext)
  
  console.log(`Atualizando XP para ${hero.nome}: ${hero.xp}/${hero.xpToNext} (${ratio}%)`)
...
function buildCard(h) {
  ...
  tabs.forEach(tab=>tab.addEventListener('click',ev=>{
    ...
    root.querySelectorAll('.ability').forEach(a=>a.classList.remove('active'))
    root.querySelector(`.ability[data-ability="${abil}"]`).classList.add('active')
  }))
  ...
  flipper.addEventListener('click',e=>{
    ...
    if(e.target.closest('.ability-tab')||e.target.closest('.ability'))return
    const flipped=root.classList.toggle('flipped')
    root.querySelectorAll('.art-placeholder img,.ability-art-placeholder img')
      .forEach(img=>img.style.transform=flipped?'scaleX(-1)':'scaleX(1)')
  })
  ...
  if(combatStats.length > 0 && h.combatStats){
    const attackStat = root.querySelector('.combat-stat.attack .combat-stat-fill')
    const spellStat = root.querySelector('.combat-stat.spell-power .combat-stat-fill')
    const defenseStat = root.querySelector('.combat-stat.defense .combat-stat-fill')
    ...
    const attackText = root.querySelector('.combat-stat.attack .combat-stat-text')
    const spellText = root.querySelector('.combat-stat.spell-power .combat-stat-text')
    const defenseText = root.querySelector('.combat-stat.defense .combat-stat-text')
  ...
  heroes.forEach(hero => {
    ...
    if(cardElement) {
      ...
      if(combatStats.length > 0 && hero.combatStats){
        const attackStat = cardElement.querySelector('.combat-stat.attack .combat-stat-fill')
        const spellStat = cardElement.querySelector('.combat-stat.spell-power .combat-stat-fill')
        const defenseStat = cardElement.querySelector('.combat-stat.defense .combat-stat-fill')
        ...
        const attackText = cardElement.querySelector('.combat-stat.attack .combat-stat-text')
        const spellText = cardElement.querySelector('.combat-stat.spell-power .combat-stat-text')
        const defenseText = cardElement.querySelector('.combat-stat.defense .combat-stat-text')
        
        if(attackText) attackText.textContent = `${hero.combatStats.attack} / 100`
        if(spellText) spellText.textContent = `${hero.combatStats.spellPower} / 100`
        if(defenseText) defenseText.textContent = `${hero.combatStats.defense} / 100`
      }
    }
  })
}
...
function fxHit(duelistEl){duelistEl.classList.add('fx-hit');setTimeout(()=>duelistEl.classList.remove('fx-hit'),450)}
...
function fxDefense(inner){inner.style.animation='defensePulse 0.6s ease-in-out';setTimeout(()=>inner.style.animation='',600)}
...
function calcularDanoBase(atkHero){
  ...
  const mod=.8+atkHero.poderDeCombate()*.4
...
function combatePorTurnos(heroiA, heroiB) {
  ...
  function aplicaDano(atk, def, abil) {
    ...
    let dmg = abilEff.damage || Math.floor(calcularDanoBase(atk.heroi) * (abilEff.mult || 1) + (abilEff.bonusFlat || 0));
  ...
  while (A.hp > 0 && B.hp > 0 && n < 80) {
    const atk1 = duo[0], def1 = duo[1];
    const atk2 = duo[1], def2 = duo[0];

    let abil1 = escolherHabilidade(atk1, def1);
    if (abil1 && abil1.cost > 0) {
      const falta1 = atk1.heroi.tipo === 'Mago' ? (atk1.mp < abil1.cost) : (atk1.sp < abil1.cost);
      if (falta1) abil1 = atk1.heroi.abilities.attack;
    }
    let abil2 = escolherHabilidade(atk2, def2);
    if (abil2 && abil2.cost > 0) {
      const falta2 = atk2.heroi.tipo === 'Mago' ? (atk2.mp < abil2.cost) : (atk2.sp < abil2.cost);
    ...
    if (chosenAbility.get(atk1.heroi.id) === abilKey1) chosenAbility.delete(atk1.heroi.id);
    if (chosenAbility.get(atk2.heroi.id) === abilKey2) chosenAbility.delete(atk2.heroi.id);
    const reg1 = {
      turno: n,
      atacante: atk1.heroi,
      defensor: def1.heroi,
    ...
    if (abil1 && abil1.type === 'def') {
      ...
      const cura = Math.floor((abil1.heal || 0.2) * (atk1.heroi.tipo === 'Mago' ? atk1.mpMax : atk1.spMax));
    ...
    if (abil2 && abil2.type === 'def') {
      ...
      const cura = Math.floor((abil2.heal || 0.2) * (atk2.heroi.tipo === 'Mago' ? atk2.mpMax : atk2.spMax));
    ...
    if (def1.hp <= 0 || def2.hp <= 0) {
      if (def1.hp <= 0) {
        turnos.push({
          turno: n, atacante: def1.heroi, defensor: def2.heroi,
          abilKey: 'death', abilName: 'Morte', tipo: 'death',
          gasto: 0, dano: 0, hpRestante: 0, hpMaxDef: def1.hpMax
        });
      }
      if (def2.hp <= 0) {
        turnos.push({
          turno: n, atacante: def2.heroi, defensor: def1.heroi,
          abilKey: 'death', abilName: 'Morte', tipo: 'death',
          gasto: 0, dano: 0, hpRestante: 0, hpMaxDef: def2.hpMax
        });
      }
      break;
    }
...
function iniciarCombate(){
  ...
  atualizarHUDCard(
    cardA,
    heroA.hpMax, heroA.hpMax,
    Math.floor(heroA.mpMax * 0.6), heroA.mpMax,
    Math.floor(heroA.spMax * 0.6), heroA.spMax
  )
  atualizarHUDCard(
    cardB,
    heroB.hpMax, heroB.hpMax,
    Math.floor(heroB.mpMax * 0.6), heroB.mpMax,
    Math.floor(heroB.spMax * 0.6), heroB.spMax
  )
  ...
  innerL.classList.remove('attack-left'); innerR.classList.remove('attack-right')
...
function executarCombatePorTurnos(res,innerL,innerR,cardA,cardB){
  let i=0
  function prox(){
    if(i>=res.turnos.length){finalizarCombate(res);return}
    ...
    ui.log([`<span class="title">Turno ${t.turno}</span>`,`${t.atacante.nome} usa <b>${t.abilName}</b>!`])
    ...
    setTimeout(()=>{
      ...
      setTimeout(()=>{
        ...
        duelL.classList.remove('on-top');duelR.classList.remove('on-top')
        i++
        positionBattleLog()
...
function atualizarParciaisHUD(res, ate, cardL, cardR) {
  ...
  const state = {
    [leftId]:  { hp: hL.hpMax, hpMax: hL.hpMax, mp: Math.floor(hL.mpMax * 0.6), mpMax: hL.mpMax, sp: Math.floor(hL.spMax * 0.6), spMax: hL.spMax },
    [rightId]: { hp: hR.hpMax, hpMax: hR.hpMax, mp: Math.floor(hR.mpMax * 0.6), mpMax: hR.mpMax, sp: Math.floor(hR.spMax * 0.6), spMax: hR.spMax }
  };

  const guard = {};
  res.turnos.slice(0, ate + 1).forEach(turn => {
    const atkId = turn.atacante.id, defId = turn.defensor.id;
    ...
    if (abil.type === 'def') {
      ...
      const cura = Math.floor((heroAtk.tipo === 'Mago' ? state[atkId].mpMax : state[atkId].spMax) * (abil.heal || 0.2));
      ...
      if (turn.abilKey === 'attack') {
        ...
        if (heroAtk.tipo === 'Mago') {
          state[atkId].mp = Math.min(state[atkId].mpMax, state[atkId].mp + energyRecovery);
        } else {
          state[atkId].sp = Math.min(state[atkId].spMax, state[atkId].sp + energyRecovery);
        }
      }
    }
  });

  atualizarHUDCard(cardL, state[leftId].hp,  state[leftId].hpMax,  state[leftId].mp,  state[leftId].mpMax,  state[leftId].sp,  state[leftId].spMax);
  atualizarHUDCard(cardR, state[rightId].hp, state[rightId].hpMax, state[rightId].mp, state[rightId].mpMax, state[rightId].sp, state[rightId].spMax);

  const heroL = heroes.find(h => h.id === leftId);
  const heroR = heroes.find(h => h.id === rightId);
  if (heroL) { heroL.hp = state[leftId].hp; heroL.mp = state[leftId].mp; heroL.sp = state[leftId].sp; }
  if (heroR) { heroR.hp = state[rightId].hp; heroR.mp = state[rightId].mp; heroR.sp = state[rightId].sp; }
...
function finalizarCombate(res){
  ui.log([`<span class="title">Resultado Final</span>`,`Vencedor: ${res.vencedor.nome}!`])
  ...
  setTimeout(()=>{
    ...
    setTimeout(()=>{
    if(res.vencedor.id===res.turnos[0].atacante.id){duelL.classList.add('winner');duelR.classList.add('loser')}
...
function resetArena(){
  ...
  ;[duelL,duelR].forEach((d,i)=>{
    d.classList.remove('winner','loser','on-top','fx-shield','fx-hit','enter-left','enter-right')
    d.style.right = ''
...
window.addEventListener('resize',()=>{if(arena.classList.contains('show')) positionBattleLog()})
